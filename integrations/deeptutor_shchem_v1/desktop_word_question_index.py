...
_QUESTION = re.compile(
    ...
    r"(?:写出|回答|解答|判断|填空|填入|填[写写出]|选择|求出|计算出|计算下列|按要求)|"
    r"(?:为什么|如何|哪些|哪项|多少|是否(?:正确|合理|成立)|的是\s*[（(])"
)
...
_HEADING = re.compile(
    ...
    r"温馨提示|方法总结|规律总结|题组[A-ZＡ-Ｚ一二三四五六七八九十]?|"
...
def _marker(text: str) -> str | None:
    ...
    match = _BRACKETED.match(text) or _BARE.match(text)
...
def _heading(text: str) -> bool:
    ...
    return bool(
        numbered
        and not _QUESTION.search(numbered.group(2))
        and _KNOWLEDGE_NUMBER.match(numbered.group(2))
    )
...
def _question_label(
    ...
    if match is None or _heading(text):
        return None
    if _QUESTION.search(match.group(2)):
    ...
    for block in blocks[position + 1 : position + 65]:
        ...
        if (
            ...
            break
        if _OPTION.search(following) or _QUESTION.search(following):
            return match.group(1)
