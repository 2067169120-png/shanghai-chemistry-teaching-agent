...
## 2026-09-19 维护源码增量（不改变已发布v0.1.101）

PR #26 已关闭旧目标比较；PR #29 已接纳可靠性修复并解除当前维护分支自动发包。X02新增独立复练题集候选：有序来源引用随考试保存，原组卷器在任务作用域内读取，既不临时替换全局题篮，也不覆盖全局当前预览。旧任务重新勾选才迁移；来源文件必须保留，失配时阻断。题集/目标改变使确认失效，原考试改分后的全链传播X03仍未完成。详见[本轮实现及验收](qa/2026-09-19-practice-set.md)。下表保留原发布版口径。
