...
当前软件源代码版本为 **0.1.60**。原教案窗口默认通读全文，支持全文查找、章节定位和本段原图对照；选段与备课另设页签，阅读不会自动扩大备课范围。0.1.59的福辛普利题面、公共材料与非官方答案接入继续保留，0.1.58的主动选择多模态备课仍可用。完整原Word始终是内容来源，不用摘要替代现成教案。本轮没有真实模型调用，未复核独立EXE启动与关闭；源码测试、冻结包检查与课堂使用反馈分别记录。具体批次见[更新记录](CHANGELOG.md)，闭环进度见[教师工作台路线图](integrations/deeptutor_shchem_v1/TEACHER_WORKBENCH_ROADMAP.md)。
...
表格目前仍按原生提取的行列文字阅读，图片按明确的区块归属展示，不声称还原Word分页、表格单元格内图片位置或旧公式。原文件入口始终可见。原文中的HTML样式字符串按文字显示，阅读器不自动打开网络链接或本机资源；换来源、图片读取失败或哈希不一致时清除旧图。
...
本轮核对了原页与22处实际显示图，以及原生预览、题篮和学生/教师内容计划。ZnS读取26项、原生集成12项、福辛普利接线9项通过；旧公众号、主题目录及题图/答案边界回归29项通过（共76个不同用例，重复执行不累加），Ruff与差异格式检查通过。未生成这组新的DOCX/PDF、未核验Word分页，也未重建独立EXE。现有0.1.60-r2桌面包不含本增量，不能将源码接通写成已安装包或课堂终验。
...
同日对完整98份又完成只读源件与缓存复核：98/98原始Word和归档SHA一致，98/98当前版本缓存可读取，原教案查找器实际列出98项，其中46项带知识导航，0条索引警告。合计44,987区块、2,900,181字符、20,041图片引用；受检原件、缓存、批次及报告前后SHA不变，没有重新导入。每份均含旧式WMF，合计8,586处，这部分仍需原Word查看。整份阅读与备课发送分开：前者不限制全文长度，后者目前最多12张不同图片、20,000字，超限不暗中删图删文。98份完整归档和正文可读不等于所有公式已在软件内保真呈现。
...
本轮连接恢复后，实际从沪科技版选必2的“教师备课授课 → 课件”打开孙卫中老师的课程包，读完新增29页静态课件、教学设计3页、学习任务单2页和课后练习4页正文。课件第24页提供必修一教材P129的定位线索，第23、25、26页展示结构示意图与其他表示的对照；尚未独立核对该教材原页，不将教具或动画画面当成正式答题图。详情页全屏授课出现登录提示，取消后可继续公开预览；本次仍未取得原PPT，不据此推断登录后的下载权限。

这次新增参照的课程编排、课堂投影、来源计数与打包检查器回归共84项通过；检查器使用内存代码归档测试，不是新EXE运行验收。原Word来源/目录/缓存另15项测试通过。没有调用模型生成新课件，不将提示词通过等同授课成品质量通过。

随后再次尝试进入《核外电子排布的表示方法》原课程包核对下载入口，正常浏览器导航超时并报`ERR_CONNECTION_CLOSED`，尚未到达课件包内容。此次新增原件下载和课例阅读均为0；不能据此判断平台是否提供下载，也没有改代理、调用隐藏接口或绕过访问限制。既有76页视觉阅读和4份设计卡不重复计数。

已有[来源与页码清单](knowledge/courseware/smartedu/first-batch-preview-catalog.json)及4份教学设计卡。新增[核外电子排布设计卡](knowledge/courseware/smartedu/electron-configuration-design-card.md)提炼同一对象的表示互译、独立练习与笔记比较表，并保留既有实验回扣、数据发现与对照论证参照。新增课题建议已接入源码，教师可以修改或删除；0.1.60-r2桌面包仍是此前版本，不包含本次新增建议。没有自动导入平台题图，未新增本地过程截图。
