...
9月12日新增源码支持原表网格预览：打开一份已导入Word时，从该原件读取明确的行列、横向合并和纵向合并关系，并逐表核对文字与当前预览一致；不改98份缓存、原文、题目标记或备课选区。顶部可切换“网格预览／逐格原文”。超过6列、嵌套或合并关系不明确时保留逐格原文并提示核对原Word；不能读取网格时全文仍可使用。图片仍按明确的区块归属展示，不声称还原Word分页、表格单元格内图片位置或旧公式。原文HTML转义，阅读器不自动打开网络链接或本机资源。此增量尚未重建独立桌面包。
...
本轮原文表格、推荐界面、原生集成、HTTP、投影与冻结检查器联合210项测试通过，含1个既有Qt警告。冻结检查器测试不等于新EXE运行；这些新增功能尚未重建独立桌面包。
...
最新现场核查为2026年9月12日18:43：当前浏览器已恢复访问，直接打开《核外电子排布的表示方法》对应课程包，确认29页课件及“教学设计、学习任务单、课后练习”入口，并再次打开3页教学设计。包内确有课件；当前未登录界面和已观察的页面资源中未发现原PPT下载入口或文件，不据此判断登录后权限。未调用隐藏接口，不重复累加已读页数。此前网络失败只作为历史记录，不再代表当前访问状态。
...
| 学生学习跟踪与推荐练习 | 原生学生页记录教师评分、诊断和章节，明确点击推荐，先看完整题目再加入题篮 | 新增源码已接通；不自动写长期学情，跨作业趋势与复练反馈尚在开发，独立包待更新 |
