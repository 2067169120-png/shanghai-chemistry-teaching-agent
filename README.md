...
当前软件源代码及本机桌面包版本为 **0.1.56**，具体批次见[更新记录](CHANGELOG.md)；根启动器已切换并验证实际运行0.1.56，旧版保留回退。本轮两主题已实际导出学生/教师DOCX及PDF，学生与教师各4页，8个不同页面已逐页查看；答案结构图和末页留白仍有待完善。完整原教案仍为主：直接提取原生文字、表格和原图，不用知识摘要替代已完成的教案。本轮把两个平台课题的教学组织参考接入可编辑备课要求，并继续整理已接通公众号题目的共同材料和依赖。标签体系、依赖感知选题、材料驱动出题与学生学习跟踪的实际进度及验收边界见[教师工作台路线图](integrations/deeptutor_shchem_v1/TEACHER_WORKBENCH_ROADMAP.md)。源码版本、桌面构建核验和课堂使用反馈分别记录。

## 在题库查看已接通的公众号题目（0.1.54接入，0.1.56源码继续修订）
...
重新预览后，共同材料会按明确关联的小题放到首次使用位置，不再全部挤在主题开头；旧记录没有成员关系时仍保留主题开头展示，不猜归属，非法或不匹配的绑定会拒绝。材料标签与图不再组成连续的共享表格；显示标题按源卷主题顺序去除重复编号，保留来源题号与教师评分对应，原来源绑定不改。第二次导出的8个不同页面已逐页查看，确认标题不再独占封面、4项材料均位于首次使用题前、松江页边杂项及重复主题编号消除，教师评分与来源题号对应。仍需完善学生末页仅第5题而留白较多，以及上中东校主题五第3、5题答案结构图未随教师答案输出的问题；不是全库整改或最终课堂成品验收。
...
0.1.56的匹配课例、可编辑表单、真实教师简报与隔离草稿存取已核对，真实Qt主视图及窄窗使用合成资料检查。当前回归证据按范围分组记录，不把重复执行或内含子集相加，也不宣称全仓库测试通过：

| 核对范围 | 当前结果 |
| --- | --- |
| 共享材料成员绑定、渲染与评分相关回归 | 130个不同用例已覆盖通过；更新旧表格布局断言后的49项复跑不另加计数 |
| 原生独立审计 | 8项通过；25项展示、9处修订、20单元题答逐字对应；52张裁片、67份受检源文件未改 |
| 裁图修订 | 44项通过 |
| 预览缓存与检查器 | 66项通过，其中包含14项检查器测试 |
| 课型与平台参照 | 26项通过 |
| 备课Provider合同 | 30项通过，不代表真实外部模型调用或内容质量验证 |
| 课堂规则与待核状态 | 分别4项、6项通过 |

各组保留JUnit证据。新增改动未引入新的Ruff问题；旧渲染器及workbench完整检查仍有HEAD已存在的1项SIM102、3项BLE001，未扩大范围修改，也不宣称全仓库Ruff全部通过。

首次导出发现排版问题后已修订并重新生成，第二次导出得到学生/教师DOCX/PDF共4文件、各4页；9张修订展示图全部使用，旧展示图被排除，74份绑定来源前后未改，模型调用0。8个不同页面已逐页实际查看，DOCX/PDF对应的8对页面PNG SHA一致，修正点和剩余缺口见上文。这是本轮两主题的实际检查，不替代全库、化学答案或课堂效果验收。

前版0.1.55的352项定向回归及14项依赖阻断、0.1.54的原生接线与旧裁图发现均保留在[更新记录](CHANGELOG.md)，不混入本版测试或实际导出结论。来源字节一致不代表源裁片干净，也不替代化学答案、评分和课堂使用核对。

本机构建标识为`0.1.56-20260910-r1`：71个冻结模块检查通过，其中70个产品模块递归代码对象与当前源码一致；ZIP全新解压后的269个文件逐个核对大小与SHA。新解压包约3.19秒出现响应窗口、1.55秒正常关闭；根启动器另行验证实际运行0.1.56，约3.18秒启动、1.46秒正常关闭。两次均使用不同的隔离个人状态和最小系统PATH，退出码0、错误日志0，未调用真实模型。这是本机构建与启动核验，不是全部界面交互、干净新机或课堂质量验收。

本机ZIP为64,176,482字节，SHA-256为`341590940acc55c3bc20201ef265af465fa7c3b7dab2aa7614ea7f76be23a260`；EXE SHA-256为`889a36b231f4d791c9343594f94a95a071494dd555265dc9dcbab853c3851f3d`。构建包不进入Git；根启动器依次尝试0.1.56、0.1.54、0.1.53，后两版保留回退。
