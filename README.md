...
当前软件源代码版本为 0.1.53，具体批次见[更新记录](CHANGELOG.md)。完整原教案仍为主：直接提取原生文字、表格和原图，不用知识摘要替代已完成的教案。本版在备课页接通原教案知识检索和本课知识点选题，并补齐“原教案区块图文一起带入备课”的链条。标签体系、依赖感知选题、材料驱动出题与学生学习跟踪的实际进度及验收边界见[教师工作台路线图](integrations/deeptutor_shchem_v1/TEACHER_WORKBENCH_ROADMAP.md)。源码版本、桌面构建核验和课堂使用反馈分别记录。
...
0.1.53对18个相关测试文件执行独立进程回归，并在最后修改后补跑受影响文件；按每文件最新JUnit结果去重，共343项通过、0失败、0错误、0跳过。覆盖原教案引用、原图、来源变化、标签建议、旧入口兼容与图文失败回滚。这是本轮针对性回归，不是全仓库测试全部通过的声明。实际个人资料另做只读验证：98份原文、46份索引均可列出，五个关键词检索和所选原图参考可读，未修改个人状态或调用模型；这些检查不等于全面化学或页面审校。

0.1.53本机构建标识为`0.1.53-20260910-r1`：61个冻结模块检查通过，其中60个产品模块递归代码对象与源码一致；ZIP全新解压后的269个文件逐个核对大小与SHA。新解压包在隔离个人状态、最小系统PATH下约2.92秒出现响应窗口、1.41秒关闭；根启动器另行验证确实指向0.1.53，约3.34秒启动、1.30秒关闭，错误日志0。上述检查不调用真实模型，也不是全部冻结界面交互或课堂质量验收。

本机ZIP的SHA-256为`ddec7d26b851de9d77de66b348b3d36545d947fe4994b069aa30134c6452da6c`。构建包不进入Git；项目根启动器优先使用0.1.53，保留0.1.52和0.1.51作为回退，更早文件未删除。
