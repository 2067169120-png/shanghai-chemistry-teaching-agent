...
当前版本为 **0.1.66**，已将所选原Word、对应讲义提炼、明确关联的教材知识和选题标签接入可预览的备课参考。现成Word的原文、表格和图片仍是主内容；知识索引用于查找和经预览后附加的讲解/笔记参考，不重写或重复导入98份教案。本轮没有真实模型调用，图片识别到个人逐题库、API语义标注和真实分页预览尚未完成，不把这些中间环节称为全功能闭环。具体见[更新记录](CHANGELOG.md)及[教师工作台路线图](integrations/deeptutor_shchem_v1/TEACHER_WORKBENCH_ROADMAP.md)。
