...
当前版本为 **0.1.63**，接通Word原生题与原卷主题的统一题篮、图文预览和双DOCX导出。现成Word的原文、表格和图片仍是教案主内容，知识索引只辅助查找，不重写或重复导入98份教案。前轮两条Word引用链的图片提示修订一并纳入：导入只保存本地素材，后续是否发送像素由生成模式和发送预览决定。本轮没有真实模型调用，独立EXE交互、DOCX成品分页与课堂效果不由源码或冻结检查代替。具体见[更新记录](CHANGELOG.md)及[教师工作台路线图](integrations/deeptutor_shchem_v1/TEACHER_WORKBENCH_ROADMAP.md)。
