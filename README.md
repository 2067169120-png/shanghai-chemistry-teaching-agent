...
当前版本为 **0.1.69**，增加长教案的多段选材、原子结构两课时讲练，并继续补现有题目标签。原教案、明确关联的教材知识与当前题目标签共用备课参考；现成Word仍是主内容，不重写或重复导入98份资料。全库标记、全书与全专题深度蒸馏、图片识别到个人逐题库和真实分页预览仍未全部完成。具体见[更新记录](CHANGELOG.md)及[教师工作台路线图](integrations/deeptutor_shchem_v1/TEACHER_WORKBENCH_ROADMAP.md)。
...
常规Word导入后的自动首轮标签仍是本地规则，API补缺须经上述入口确认，不等于所有上传资料已全自动处理。已有错误标签应在“修改教学标签”另行核对，本入口不会为补缺覆盖已有值。当前目录对部分基础电解质主题仍不够细，不能为消除未知而强配进阶章节。
