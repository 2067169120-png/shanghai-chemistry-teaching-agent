...
当前软件版本为 0.1.49，具体批次见[更新记录](CHANGELOG.md)。本次新增已导入 Word 的逐题浏览、勾选、范围修订、带入备课和原生 Word 练习导出。标签体系、依赖感知选题、材料驱动出题与学生学习跟踪的实际进度及验收边界见[教师工作台路线图](integrations/deeptutor_shchem_v1/TEACHER_WORKBENCH_ROADMAP.md)。提示词要求与已经实现的功能分开记录。
...
6. 用于教案或PPT：先点“预览选题”，核对完整参考后点“确认带入备课”。所选文字追加到现有资料，不清空已填课题、课时和要求。**这一步只追加文字；题中含图时会提示另行附加对应图片，不假称模型已经看过原图。**
...
| 学生学习跟踪与推荐练习 | 按具体作答、知识点标签与复练结果衔接题库推荐 | 完整自动跟踪及推荐闭环尚在开发，不把规划写成可用功能 |
