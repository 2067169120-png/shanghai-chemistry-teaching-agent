...
当前版本为 **0.1.65**，在可勾选叠加筛选和统一题篮基础上，补齐一批现有Word题的缺失考点/教材映射，并将两个专题的教材读图与讲义方法补充接入教案检索。现成Word的原文、表格和图片仍是教案主内容，知识索引只辅助查找，不重写或重复导入98份教案。本轮没有真实模型调用，图片识别到个人逐题库、API语义标注和真实分页预览尚未完成，不把这些中间环节称为全功能闭环。具体见[更新记录](CHANGELOG.md)及[教师工作台路线图](integrations/deeptutor_shchem_v1/TEACHER_WORKBENCH_ROADMAP.md)。
