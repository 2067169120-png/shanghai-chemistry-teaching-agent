...
当前软件源代码版本为 0.1.54，具体批次见[更新记录](CHANGELOG.md)。完整原教案仍为主：直接提取原生文字、表格和原图，不用知识摘要替代已完成的教案。本版接通两组已有公众号题目的主题图文预览，并修复原生题库中“搜到待归属小问却打不开”的卡片问题。标签体系、依赖感知选题、材料驱动出题与学生学习跟踪的实际进度及验收边界见[教师工作台路线图](integrations/deeptutor_shchem_v1/TEACHER_WORKBENCH_ROADMAP.md)。源码版本、桌面构建核验和课堂使用反馈分别记录。
...
本版是来源预览接通，不是裁图返工全部完成。最后检查27张不同题面/共享图时发现5处旧裁片缺陷：松江主题二第8题下沿混入第9题边缘；上中东校主题五《有机合成药物》第1题首行上沿被切，第2、3、4题下沿混入下一题边缘。题号按原页与来源记录核对，不按内部ID推断。已列为下一阶段优先修复，原裁片暂未替换，使用相关题目前应打开原页核对，不能仅凭当前裁图直接用于课堂或导出。另有源页页脚及重复全页共享材料保留，尚未整理成全部干净的纯材料图。
...
0.1.54对13个相关测试文件执行独立进程回归，按每文件最新JUnit结果去重，共203项通过、0失败、0错误、0跳过。覆盖来源读取器、题答角色和哈希漂移拒绝、原生搜索与待归属提示、每窗口校验、题篮/组卷材料保留和冻结包检查；真实本地集成测试显式启用。该结果是针对性回归，不是全仓库测试全部通过的声明。
