...
当前版本为 **0.1.67**，新增原Word逐题API标签补缺：先查看原题、公共材料与图片，再确认调用所选模型，核对结果后勾选保存。原教案、讲义提炼、明确关联的教材知识和当前题目标签继续共用备课参考。现成Word仍是主内容，不重写或重复导入98份资料。API补缺已有少量真实文字及图片验证，但全库标记、深度蒸馏、图片识别到个人逐题库和真实分页预览仍未全部完成。具体见[更新记录](CHANGELOG.md)及[教师工作台路线图](integrations/deeptutor_shchem_v1/TEACHER_WORKBENCH_ROADMAP.md)。
...
2026年9月13日通过现有模型配置实际发起11次请求，其中1次带真实题图；10道获得可采用结果并保存，1道返回不可用结果、保持原状。新增10条主考点和7条教材映射，均为自动建议、未作教师审核。保存后重读核对来源和历史，其余1349条原缺项记录及原考试信息未改。当前98份Word的3573道候选中，**2680道有主考点，2490道有教材映射；仍缺893/1083条，两类重叠**。原考试类型839条有线索、2734条未知不变。
...
本轮不新增教材/讲义知识量：仍有383条教材概念索引，46份讲义索引含262条知识、54条方法及60条提醒；52份其他练习/测试资料保留完整原Word。原子结构和电离平衡之外的专题深化、原图/旧公式读取和全书逐页整理仍需继续，索引覆盖不代表完成课堂级蒸馏。
