...
当前版本为 **0.1.64**，在统一题篮和混合DOCX导出基础上，补充常规Word上传后的本地自动标签及可勾选叠加的教材/知识/考试条件筛选。现成Word的原文、表格和图片仍是教案主内容，知识索引只辅助查找，不重写或重复导入98份教案。本轮没有真实模型调用，图片识别到个人逐题库、API语义标注和真实分页预览尚未完成，不把这些中间环节称为全功能闭环。具体见[更新记录](CHANGELOG.md)及[教师工作台路线图](integrations/deeptutor_shchem_v1/TEACHER_WORKBENCH_ROADMAP.md)。
