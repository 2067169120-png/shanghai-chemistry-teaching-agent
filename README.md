...
当前版本为 **0.1.70**，在默认补缺之外增加已有自动标签的明确核对与纠偏，并继续使用原Word、多段选材和两课时讲练参考。原教案、明确关联的教材知识与当前题目标签共用备课参考；现成Word仍是主内容，不重写或重复导入98份资料。全库标记、全书与全专题深度蒸馏、图片识别到个人逐题库和真实分页预览仍未全部完成。具体见[更新记录](CHANGELOG.md)及[教师工作台路线图](integrations/deeptutor_shchem_v1/TEACHER_WORKBENCH_ROADMAP.md)。

## 最新资料进度：氧化还原讲练与图文题标签（2026年9月13日）
