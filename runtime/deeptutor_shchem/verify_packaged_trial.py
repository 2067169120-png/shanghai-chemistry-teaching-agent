...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='shchem-detached-') as temporary:
        ...
        from typography_ci import scaled_runs
        original_environ = os.environ.copy()
        try:
            os.environ.clear()
            os.environ.update(native_env)
            scaled_runs(qa / 'font-scaling', executable=exe)
        finally:
            os.environ.clear()
            os.environ.update(original_environ)
        report['font_scaling'] = json.loads((qa / 'font-scaling/scaling-summary.json').read_text(encoding='utf-8'))
        scan_target = qa / 'scan'
        scan_run = subprocess.run([str(exe), '--verify-scan-numbers', str(scan_target)],cwd=temp,env=env,timeout=240)
        for file in (temp/'profile').rglob('startup-error.log'):
            shutil.copyfile(file,qa/'scan-startup-error.log')
        assert scan_run.returncode == 0, 'Packaged scan-number workflow failed'
        report['scan_numbering'] = json.loads((scan_target/'scan-probe.json').read_text(encoding='utf-8'))
        explorer_target = qa / 'explorer'
        explorer_run = subprocess.run([str(exe), '--verify-explorer', str(explorer_target)], cwd=temp, env=env, timeout=180)
        for file in (temp/'profile').rglob('startup-error.log'):
            shutil.copyfile(file,qa/'explorer-startup-error.log')
        assert explorer_run.returncode == 0, 'Packaged question-page workflow failed'
        report['explorer_performance'] = json.loads((explorer_target/'explorer-probe.json').read_text(encoding='utf-8'))
        review_target = qa / 'student-review'
        review_run = subprocess.run([str(exe), '--verify-student-review', str(review_target)], cwd=temp, env=env, timeout=240)
        for file in (temp/'profile').rglob('startup-error.log'):
            shutil.copyfile(file,qa/'review-startup-error.log')
        assert review_run.returncode == 0, 'Packaged student-review workflow failed'
        report['student_review'] = json.loads((review_target/'student-review-probe.json').read_text(encoding='utf-8'))
        batch_target = qa / 'work-batch'
        batch_run = subprocess.run([str(exe), '--verify-work-batch', str(batch_target)], cwd=temp, env=env, timeout=180)
        for file in (temp/'profile').rglob('startup-error.log'):
        ...
        closure_target = qa / 'closure'
        closure_run = subprocess.run([str(exe), '--verify-closure', str(closure_target)], cwd=temp, env=env, timeout=300)
        for file in (temp/'profile').rglob('startup-error.log'):
            shutil.copyfile(file,qa/'closure-startup-error.log')
        assert closure_run.returncode == 0, 'Packaged teacher-score / real-library / retest workflow failed'
        report['teaching_closure'] = json.loads((closure_target/'closure-probe.json').read_text(encoding='utf-8'))
        lesson_target = qa / 'lesson-design'
        lesson_run = subprocess.run([str(exe), '--verify-lesson-design', str(lesson_target)],
                                    cwd=temp, env=env, timeout=300)
        for file in (temp/'profile').rglob('startup-error.log'):
            shutil.copyfile(file,qa/'lesson-startup-error.log')
        assert lesson_run.returncode == 0, 'Packaged lesson design and linked-output workflow failed'
        report['lesson_design'] = json.loads((lesson_target/'lesson-design-probe.json').read_text(encoding='utf-8'))
        imported_target = qa / 'lesson-import'
        imported_run = subprocess.run([str(exe), '--verify-lesson-import', str(imported_target)], cwd=temp, env=env, timeout=300)
        for file in (temp/'profile').rglob('startup-error.log'):
            shutil.copyfile(file,qa/'lesson-import-startup-error.log')
        assert imported_run.returncode == 0, 'Packaged existing-result import and restored lesson outputs failed'
        report['lesson_import'] = json.loads((imported_target/'lesson-import-probe.json').read_text(encoding='utf-8'))
    ...
    (package/'BUILD.json').write_text(json.dumps({'version':'0.1.101','source_commit':os.environ.get('SHCHEM_SOURCE_SHA', os.environ['GITHUB_SHA']),
    ...
    shutil.make_archive(str(release/'ShanghaiChem-0.1.101-Windows-x64'),'zip',package.parent,package.name)
